...
- Only **12 out of 106** circuits scored above 0. The remaining **94 scored 0/8** — their circuits are not statistically distinguishable from the population average at any layer.
- **All top-scoring circuits are AST nodes:** `Import` (7/8), `Break` (6/8), `Pass` (6/8), `ImportFrom` (5/8), `Continue` (4/8). These are structurally simple, syntactically distinctive constructs.
- **No builtin scored above 1/8.** Only `float` and `_isolated_` reached 1/8. All other builtins — types, functions, exceptions — scored 0/8.
- AST nodes have a mean modularity score of 0.8; builtins have a mean score of 0.0.
